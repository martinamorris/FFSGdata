...
common_columns = c("source",
                   "age", 
                   "race",
                   "sex") # Name & Data are also common, but won't be tabled

all_datasets = rbind(mpv[, common_columns], 
                 kbp[, common_columns], 
                  fe[, common_columns])

for (column in common_columns[-1]) {
    print(table(all_datasets[, c('source', column)]), usNA='always')
}
```
...
```{r}
# Binned Age
all_datasets[, 'age_bin'] = as.integer(as.numeric(all_datasets$age) / 10)
print(table(all_datasets[, c('source', 'age_bin')]), usNA='always')

# Ages that would need parsed / interpreted
all_datasets[, 'simple_age'] =  grepl("^[0-9]{1,2}$", all_datasets$age)
print(table(all_datasets[, c('source', 'simple_age')]), usNA='always')
```
