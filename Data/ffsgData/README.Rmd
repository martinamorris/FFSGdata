...
* type, deduplication
* M , U, W are the W = log(M/U) are the weights returned by the algorithm
* Wdata, 
...
* FE : `r round(nrow(filter(fe_harmonized, is.na(race))) / nrow(fe_harmonized) * 100, 2)`

* KBP : `r  round(nrow(filter(kbp_harmonized, is.na(race))) / nrow(kbp_harmonized) * 100, 2)`

* WAPO : `r round(nrow(filter(wapo_harmonized, is.na(race))) / nrow(wapo_harmonized) * 100, 2) `

* MPV : `r round(nrow(filter(mpv_harmonized, is.na(race))) / nrow(mpv_harmonized) * 100, 2)`


Missing race after linkage:
Final dataset: `r round(nrow(filter(final_merged, is.na(Race)))/ nrow(final_merged) * 100, 2) `
