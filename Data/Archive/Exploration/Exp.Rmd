...
path2 = here::here(file.path('Data','ffsgData' ,'R', 'Harmonizing', 'HarmonizedFiles'))
...
p.raw <- plot_ly(data, x = ~dfs, y = ~before_2013, type = 'bar', name = 'Before 2013',
...
#harmonized data
hfe_before_2013 <- nrow(filter(fe_harmonized, fe_harmonized$year < 2013))
hfe_after_2013 <- nrow(filter(fe_harmonized, fe_harmonized$year >= 2013))
hkbp_before_2013 <- nrow(filter(kbp_harmonized, year(kbp_harmonized$date) < 2013))
hkbp_after_2013 <- nrow(filter(kbp_harmonized, year(kbp_harmonized$date) >= 2013))
hwp_before_2013 <- nrow(filter(wapo_harmonized, year(wapo_harmonized$date)< 2013 ))
hwp_after_2013 <- nrow(filter(wapo_harmonized, year(wapo_harmonized$date) >= 2013 ))
hmpv_before_2013 <-  nrow(filter(mpv_harmonized, year(mpv_harmonized$date)< 2013 ))
hmpv_after_2013 <-  nrow(filter(mpv_harmonized, year(mpv_harmonized$date) >= 2013 ))
hbefore_2013 <- c(hfe_before_2013, hkbp_before_2013, hwp_before_2013, hmpv_before_2013 )
hafter_2013 <- c(hfe_after_2013, hkbp_after_2013, hwp_after_2013, hmpv_after_2013)
hdfs <- c("Fatal encounters", "Killed by Police", "Washington Post", "Mapping Police Violence")
hdata <- data.frame(hdfs, hbefore_2013, hafter_2013)
p.har <- plot_ly(hdata, x = ~hdfs, y = ~hbefore_2013, type = 'bar', name = 'Before 2013',
        marker = list(color = 'rgb(55, 83, 109)')) %>%
         add_trace(y = ~hafter_2013, name = 'After 2013', marker = list(color = 'rgb(26,          118, 255)')) %>%
         layout(title = "Number of records", yaxis = list(title = "Number of records"),           xaxis = list(title = "Harmonized Dataset"), barmode = 'group', bargap = 0.15)
p.har
...
fe_harmonized <- filter(fe.clean, fe.clean$year >= 2013)
```
...
Total age records for raw data are `r nrow(fe.clean)` .
...
Raw data : `r nrow(filter(fe.clean, is.na(fe.clean$age) | fe.clean$age == ""))`
...
```{r echo = FALSE}
p.raw <- plot_ly(x = ~fe.clean$age, type = "histogram") %>% 
...
p.raw <- plot_ly(x = ~as.factor(kbp$Age), type = "histogram")  %>% 
...
Of the non-missing, following is the histogram of occurences:
```{r echo = FALSE, warning=  FALSE}
p.raw <- plot_ly(x = ~as.factor(wapo$age),type = "histogram")   %>% 
  layout( xaxis = list(title = "Washington post, Raw dataset"))
p.raw

p.har <- plot_ly(x = ~as.factor(wapo_harmonized$age), type = "histogram")  %>% 
...
Raw data : 
```{r}
nrow(filter(mpv, is.na(mpv$`Victim's age`) |mpv$`Victim's age` == ""))
```
...
p.raw <- plot_ly(x = ~as.factor(mpv$`Victim's age`), type = "histogram")  %>% 
  layout( xaxis = list(title = "Mapping Police Violence, Raw data"))
p.raw

p.har <- plot_ly(x = ~as.factor(mpv_harmonized$age), name = , type = "histogram")  %>% 
...
p.raw <- plot_ly(alpha = 0.6) %>%
  add_histogram(x = ~fe.clean$age, name = "Fatal encounters") %>%
  add_histogram(x = ~as.factor(kbp$Age), name = "Killed by Police") %>%
  add_histogram(x = ~as.factor(wapo$age), name = "Washington post") %>%
   add_histogram(x = ~as.factor(mpv$`Victim's age`), name = "Mapping Police Violence") %>%
...
p.har <- plot_ly(alpha = 0.6) %>%
  ...
  add_histogram(x = ~as.factor(kbp_harmonized$age), name = "Killed by Police") %>%
  add_histogram(x = ~as.factor(wapo_harmonized$age), name = "Washington post") %>%
   add_histogram(x = ~as.factor(mpv_harmonized$age), name = "Mapping Police Violence") %>%
...
Total missing age records are: 
Raw data: `r nrow(filter(fe.clean, is.na(fe.clean$race) | fe.clean$race == "")) ` 
Harmonized data: `r nrow(filter(fe_harmonized, is.na(fe_harmonized$race) | fe_harmonized$race == "")) `


Of the non-missing, following is the histogram of occurences.
```{r echo = FALSE}
p.raw <- plot_ly(x = ~fe.clean$race, type = "histogram")  %>% 
  layout( xaxis = list(title ="Fatal encounters, raw dataset"))
p.raw

p.har <- plot_ly(x = ~fe_harmonized$race, type = "histogram")  %>% 
...
p.har <- plot_ly(x = ~kbp_harmonized$race, type = "histogram")  %>%  
...
Raw data :
```{r  echo = FALSE }
nrow(filter(mpv, is.na(mpv$`Victim's race`) |  mpv$`Victim's race` == ""))
...
p <- plot_ly(x = ~mpv$`Victim's race`, type = "histogram")  %>%  
  layout( xaxis = list(title =   "Mapping Police Violence, raw data"))
p

p.har <- plot_ly(x = ~mpv_harmonized$race, name = , type = "histogram")  %>%  
  layout( xaxis = list(title = "Mapping Police Violence, harmonized data"))
p.har
```
...
Total missing age records are
Raw data: `r  nrow(filter(wapo, is.na(wapo$race) | wapo$race == "")) `
Harmonized data: `r nrow(filter(wapo_harmonized, is.na(wapo_harmonized$race) | wapo_harmonized$race == ""))`

Of the non-missing, following is the histogram of occurences.
```{r echo = FALSE, warning= FALSE}
p.raw <- plot_ly(x = ~wapo$race, type = "histogram") %>%  
  layout( xaxis = list(title =   "Washington Post, raw data"))
p.raw

p.har <- plot_ly(x = ~wapo_harmonized$race,type = "histogram")  %>%  
...
p.raw <- plot_ly(alpha = 0.6) %>%
  add_histogram(x = ~fe.clean$race, name = "Fatal encounters") %>%
