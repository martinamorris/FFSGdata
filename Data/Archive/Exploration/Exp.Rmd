...
fe.clean$age = fe.clean$age %>%  replace_na("Missing")
p.raw <- plot_ly(x = ~fe.clean$age, type = "histogram") %>% 
  layout( xaxis = list( title = "Fatal encounters, Raw dataset"))
p.raw

fe_harmonized$age = fe_harmonized$age %>%  replace_na("Missing")
...
kbp$Age = kbp$Age %>% replace_na("Missing")
...
Following is the histogram of occurences:
```{r}
mpv$`Victim's age`= as.character(mpv$`Victim's age`) %>% replace_na("Missing")
...
Total missing race records are: 
...
Following is the histogram of occurences.
```{r}
fe.clean$race<- replace(fe.clean$race, is.na(fe.clean$race), "NA")
p.raw <- plot_ly(x = ~fe.clean$race, type = "histogram", color =  ~fe.clean$race)  %>% 
  layout( xaxis = list(title ="Fatal encounters, raw dataset"))
p.raw
...
kbp_harmonized$race = kbp_harmonized$race %>%   replace_na("Missing")
...
Following is the histogram of occurences.
```{r}
mpv$`Victim's race` = tolower(as.character(mpv$`Victim's race`) )%>%   replace_na("Missing")
p <- plot_ly(x = ~mpv$`Victim's race`, type = "histogram", color = ~mpv$`Victim's race`)  %>%  
  layout( xaxis = list(title =   "Mapping Police Violence, raw data"))
p

mpv_harmonized$race = as.character(mpv_harmonized$race) %>%   replace_na("Missing")
