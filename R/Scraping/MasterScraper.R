...
path_to_save = here::here(file.path('Data', 'ScrapedFiles'))

scrape_all_data <- function() {

    # save cleaned copy
    if(!file.exists(file.path(path_to_save))) {
        dir.create(file.path(path_to_save), recursive=T)
    }
    ...
    pop_save_dir = file.path(path_to_save, "Populations")
    ...
    fe_url = sub("(*.)DOC_ID(*.)", paste("\\1", doc_id, "\\2", sep=""), url_template)

    fe_save_file = file.path(path_to_save, "fe.clean.Rdata")
    ...
    kbp_save_file = file.path(path_to_save, "KBP.clean.Rdata")
    ...
    mpv_save_file = file.path(path_to_save, "MPV.clean.Rdata")
    ...
    wapo_save_file = file.path(path_to_save, "WaPo.clean.Rdata")
